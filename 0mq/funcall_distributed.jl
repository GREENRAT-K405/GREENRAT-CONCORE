# funcall_distributed.jl
# Julia port of funcall_distributed.py
# ZMQ REQ client for distributed (cross-machine) deployment.
# Connects to a remote IP address rather than localhost.
# PORT_NAME_IN_F1 and PORT_IN_F1 are injected by mkconcore at deployment time.
# REMOTE_IP should also be injected or hardcoded for the target deployment.
...
# Connect to the remote machine running funbody_distributed.jl
# Replace REMOTE_IP with the actual IP, e.g. "192.168.0.107"
init_zmq_port(PORT_NAME_IN_F1, "connect", "tcp://" * REMOTE_IP * ":" * PORT_IN_F1, "REQ")
