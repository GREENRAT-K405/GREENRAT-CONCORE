...
function _init_zmq_port_impl(port_name::String, port_type::String, address::String, resolved_type::Int)
    try
        # On Windows, creating a socket on the shared context and then setting
        # rcvtimeo/sndtimeo before bind hangs the process. Use a fresh context
        # per socket instead so the timeout options are set on a clean context.
        ctx = Sys.iswindows() ? ZMQ.Context() : state.zmq_ctx
        sock = ZMQ.Socket(ctx, resolved_type)
        # Set linger=0 first so close() is always fast
...
function terminate_zmq()
    ...
    for (name, port) in state.zmq_ports
        ...
        # Close per-socket context (on Windows), but not the shared state.zmq_ctx twice
