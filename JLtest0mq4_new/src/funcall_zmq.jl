...
while state.simtime < state.maxtime
    if haskey(state.iport, "U")
        # Full wired mode: wait for upstream file-port U (e.g. from cpymax) to update
        while unchanged()
            global u = concore_read(state.iport["U"], "u", init_simtime_u_str)
        end
    else
        # Standalone ZMQ mode: no cpymax upstream — mock u data and tick simtime
        sleep(1.0)
        state.simtime += 1.0
        global u = [Float64(state.simtime), 2.0, 3.0]
    end
